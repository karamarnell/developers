...
- [Custom Nginx configuration & embedding Qordoba](#custom-nginx-configuration-embedding-qordoba)
  - [Custom Nginx configuration](#custom-nginx-configuration)
  - [Embedding Qordoba in OpenResty](#embedding-qordoba-in-openresty)
  - [Serving both a website and your APIs from Qordoba](#serving-both-a-website-and-your-apis-from-qordoba)
...
Qordoba comes with a default configuration file that can be found at
`/etc/qordoba/qordoba.conf.default` if you installed Qordoba via one of the official
packages. To start configuring Qordoba, you can copy this file:

```
$ cp /etc/qordoba/qordoba.conf.default /etc/qordoba/qordoba.conf
```

Qordoba will operate with default settings should all the values in your
configuration be commented-out. Upon starting, Qordoba looks for several
default locations that might contain a configuration file:

```
/etc/qordoba/qordoba.conf
/etc/qordoba.conf
```
...
```
$ qordoba start --conf /path/to/qordoba.conf
```
...
```
$ qordoba check <path/to/qordoba.conf>
configuration at <path/to/qordoba.conf> is valid
```
...
as to what properties Qordoba is being started with:

```
$ qordoba start -c <qordoba.conf> --vv
2016/08/11 14:53:36 [verbose] no config file found at /etc/qordoba.conf
2016/08/11 14:53:36 [verbose] no config file found at /etc/qordoba/qordoba.conf
...
When loading properties out of a configuration file, Qordoba will also look for
environment variables of the same name. This allows you to fully configure Qordoba
...
```
log_level = debug # in qordoba.conf
```
...
you must tweak global settings that are not exposed by the Qordoba configuration,
...
worker_processes ${{ "{{NGINX_WORKER_PROCESSES" }}}}; # can be set by qordoba.conf
daemon ${{ "{{NGINX_DAEMON" }}}};                     # can be set by qordoba.conf

pid pids/nginx.pid;                      # this setting is mandatory
error_log logs/error.log ${{ "{{LOG_LEVEL" }}}}; # can be set by qordoba.conf
...
http {
    # include default Qordoba Nginx config
    include 'nginx-qordoba.conf';
...
You can then start Qordoba with:

```
$ qordoba start -c qordoba.conf --nginx-conf custom_nginx.template
```

If you wish to customize the Qordoba Nginx sub-configuration file to, eventually,
add other Lua handlers or customize the `lua_*` directives, you can inline the
`nginx_qordoba.lua` configuration in this `custom_nginx.template` example file:
...
#### Embedding Qordoba in OpenResty

If you are running your own OpenResty servers, you can also easily embed Qordoba
by including the Qordoba Nginx sub-configuration using the `include` directive
(similar to the examples of the previous section). If you have a valid
top-level NGINX configuration that simply includes the Qordoba-specific
configuration:
...
And Qordoba will be running in that instance (as configured in `nginx-qordoba.conf`).

[Back to TOC](#table-of-contents)

#### Serving both a website and your APIs from Qordoba

A common use case for API providers is to make Qordoba serve both a website
...
Nginx configuration template which inlines `nginx_qordoba.lua` and adds a new
`location` block serving the website alongside the Qordoba Proxy `location`
block:
...
http {
  # here, we inline the contents of nginx_qordoba.lua
  charset UTF-8;

  # any contents until Qordoba's Proxy server block
  ...

  # Qordoba's Proxy server block
  server {
    server_name qordoba;
    ...
    # Qordoba's Proxy location / has been changed to /api/v1
    location /api/v1 {
  ...
  # Qordoba's Admin server block goes below
}
```
...
and logs. Each Qordoba process must have a separate working directory.

Default: `/usr/local/qordoba`
...
property to load custom plugins that are not bundled with Qordoba. Plugins will
be loaded from the `qordoba.plugins.{name}.*` namespace.
...
Send anonymous usage data such as error stack traces to help improve Qordoba.
...
Address and port on which Qordoba will accept HTTP requests. This is the
public-facing entrypoint of Qordoba, to which your consumers will make
requests to.
...
Address and port on which Qordoba will accept HTTPS requests if `ssl` is enabled.
...
Address and port on which Qordoba will expose an entrypoint to the Admin API.
This API lets you configure and manage Qordoba, and should be kept private
and secured.
...
Address and port on which Qordoba will accept HTTPS requests to the Admin API if
...
Mainly useful for development or when running Qordoba inside a Docker environment.
...
`proxy_listen_ssl` address. If none is specified and `ssl` is enabled, Qordoba will
...
Qordoba will generate a default certificate and key.
...
Enables or disables emitting Qordoba version on error pages and in the `Server`
...
Enables or disables emitting Qordoba latency information in the `X-Qordoba-Proxy-Latency`
and `X-Qordoba-Upstream-Latency` response header fields.
...
`X-Forwarded-*` headers. Requests from trusted IPs make Qordoba forward their
`X-Forwarded-*` headers upstream. Non-trusted requests make Qordoba insert its own
`X-Forwarded-*` headers.
...
Defines the maximum request body size allowed by requests proxied by Qordoba, specified in the
Content-Length request header. If a request exceeds this limit, Qordoba will respond with a
...
buffered to disk Qordoba plugins that access or manipulate the request body may not work, so
...
Qordoba will store all of its data (such as APIs, Consumers and Plugins) in
either Cassandra or PostgreSQL.

All Qordoba nodes belonging to the same cluster must connect themselves to the
same database.

<div class="alert alert-warning">
  <p><strong>Starting with Qordoba 0.12.0:</strong></p>
  <ul>
...
**cassandra_local_datacenter**  | When using the `DCAwareRoundRobin` policy, you must specify the name of the cluster local (closest) to this Qordoba node.
...
In order to avoid unecessary communication with the datastore, Qordoba caches
...
This section allows for configuring the behavior of Qordoba regarding the
...
time taken by Qordoba to propagate the change
of an entity.
...
Qordoba will resolve hostnames as either `SRV` or `A` records (in that order, and
...
Qordoba. If not specified the nameservers in
...
in memory. To read the file again after modifying it, Qordoba must be reloaded.
...
format. This certificate will be the one used for verifying Qordoba's database
...
This includes the certificates configured for Qordoba's database connections.
