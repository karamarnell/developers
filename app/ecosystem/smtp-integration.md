...
and hence is an anti-pattern for Qordoba installations.
...
`config.path`                 |         | The file path of the output log file. The plugin will create the file if it doesn't exist yet. Make sure Qordoba has write permissions to this file.
`reopen`                      | `false` | Introduced in Qordoba `0.10.2`. Determines whether the log file is closed and reopened on every request. If the file is not reopened, and has been removed/rotated, the plugin will keep writing to the stale file descriptor, and hence lose information.
